...
from metrics import di_co, batch, mean, iou_sc
import losses
...
def parse_args():
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='AACUnet',
                        choices=arch_names,
...
def train(args, train_loader, model, criterion, optimizer, epoch, scheduler=None):
    ...
    for i, (input, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if args.deepsupervision:
            ...
            loss /= len(outputs)
            iou = iou_sc(outputs[-1], target)
        else:
            output = model(input)
            loss = criterion(output, target)
            iou = iou_sc(output, target)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        tmp = pd.Series([epoch, args.lr, train_log['loss'], train_log['iou'], val_log['loss'], val_log['iou'],
