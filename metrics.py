...
def mean(y_true_in, y_pred_in, print_table=False):
    if True:
        labels = y_true_in
        y_pred = y_pred_in
        true_objects = 2
...
def batch(output, target):
    ...
    for i in range(output.shape[0]):
        ious.append(mean(output[i], target[i]))
    return np.mean(ious)
